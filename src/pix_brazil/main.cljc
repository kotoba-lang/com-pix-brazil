(ns pix_brazil.main
  "Kotodama WASM entrypoint for the Pix_brazil clean-room actor (L4) — Clojure port.
  ...
  facts (`pix_brazil.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "pix_brazil")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"pix_brazil.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "pix_brazil-compat" :tier tier :entities entities} 200])
...
(defn start! [] :pix_brazil-compat/ready)
